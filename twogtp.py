...
# modified by Cheng Ye
# add parameters "cputime" and "movenumber" to genmove command
# add cputimeleft to player
# 2014.09.27
...
class GTP_player:
    ...
    def __init__(self, command):
        global g_cputimelimit
        if debug:
            print "command:: ",  command
        self.connection = GTP_connection(command)
        self.movenumber = 0
        self.cputimeleft = g_cputimelimit
        ...
        if debug:
            print "set protocol version to :", self.protocol_version
    ...
    def genmove(self, color):
        ...
        if self.protocol_version == "1":
            command = "genmove_" + command + " " + str(self.cputimeleft) + " " + str(self.movenumber)
        else:
            command = "genmove " + command + " " + str(self.cputimeleft) + " " + str(self.movenumber)
    ...
    def cputime(self):
        if (self.is_known_command("cputime")):
            if debug:
                print "known cputime ", self.is_known_command("cputime")
            return self.connection.exec_cmd("cputime")
        else:
            if debug:
                print "unknown cputime ", self.is_known_command("cputime")
            return "0"
...
class GTP_game:
    ...
    def __init__(self, whitecommand, blackcommand, size, komi, handicap,
        ...
        global g_cputimelimit;
        self.cputimelimit = g_cputimelimit;
    ...
    def play(self, sgffile):
        ...
        global debug
        if debug:
            print "in play function of Game: ", self
        if verbose >= 1:
        ...
        while passes < 2:
            if to_play == "B":
                ...
                self.blackplayer.movenumber += 1
                self.blackplayer.cputimeleft -= (end - start)
                if self.blackplayer.cputimeleft <= 0:
                    sys.stderr.write("black out of time.\n")
                    self.quit()
                    sys.exit(1)
                if debug:
                ...
                if move[:5] == "ERROR":
                    ...
                    if debug:
                        print "move:: ", move
                        sys.stderr.write("genmove black error.\n")
                    sys.exit(1)
                if move[:6] == "resign":
                    if verbose >= 1:
                        print "Black resigns"
                ...
                self.whiteplayer.movenumber += 1
                self.whiteplayer.cputimeleft -= (end - start)
                if self.whiteplayer.cputimeleft <= 0:
                    sys.stderr.write("white out of time.\n")
                    self.quit()
                    sys.exit(1)
                if debug:
                ...
                if move[:5] == "ERROR":
                    # FIXME: write_sgf
                    if debug:
                        sys.stderr.write("genmove white error.\n")
                    sys.exit(1)
...
class GTP_match:
    ...
    def play(self, games, sgfbase):
        if debug:
            print "I am the gtp engine play"
        last_color = ""
        last_streak = 0
